...
class CrearPaginaController {
      ...
      if (isClassLike) {
        ...
        const processedAttributes = (rawAttributes || []).map(attr => {
          if (typeof attr === 'string') {
            ...
            const tipoDiagrama = parts[1] || 'String';
            ...
            return { name, type, sqlType, visibility, defaultValue, isPrimaryKey: name.toLowerCase() === 'id' };
          }
          return {
  ...
  exportarSQLDesdeSala = async (req, res) => {
    ...
    try {
      ...
      let salaData;
      try { salaData = JSON.parse(sala.xml); } catch (err) { return response(res, 400, { error: 'El contenido de la sala no es válido' }); }
      ...
      if (elements.length === 0) return response(res, 400, { error: 'No hay clases en el diagrama para generar el modelo relacional' });
      ...
      return response(res, 500, { error: 'Error generando el script SQL', detalles: error?.message || String(error) });
    }
  };
