...
import { clienteGemini, hayClaveGemini } from '../libs/geminiRotacion.js';
import fs from 'fs';
...
class AIController {
    ...
    static async _generateUsingModels(modelsList, prompt) {
        ...
        let lastError = null;
        // Pocos reintentos por modelo: ante un 503 conviene caer rápido al siguiente
        // modelo de la lista en vez de insistir con backoff largo sobre el saturado.
        const retriesPerModel = parseInt(process.env.GEMINI_RETRIES_PER_MODEL || '2', 10);
        for (const m of arr) {
            try {
                const text = await AIController._tryGenerateWithModel(m, prompt, retriesPerModel);
                return { model: m, text };
            } catch (err) {
                lastError = err;
    ...
    static async generateDiagram(req, res) {
        try {
            ...
            res.status(500).json({
                success: false,
                error: mensajeErrorIA(error, 'No se pudo generar el diagrama con la IA. Intenta de nuevo en unos segundos.')
            });
        }
    }
    ...
    static async getAIFeatures(req, res) {
        try {
            ...
            console.error('Error listando capacidades de IA:', error);
            res.status(500).json({
                success: false,
                error: mensajeErrorIA(error, 'No se pudo consultar el estado de la IA.')
            });
        }
    }
    ...
    static async verifyDiagram(req, res) {
        try {
            ...
            if (!systemPrompt || !userPrompt) {
                return res.status(400).json({ success: false, error: 'Falta el diagrama a verificar.' });
            }

            if (!hayClaveGemini()) {
                return res.status(400).json({ success: false, error: 'La verificación con IA en la nube no está disponible porque el servidor no tiene una clave de Gemini.' });
            }
            ...
            try {
                ...
                return res.status(502).json({ success: false, error: mensajeErrorIA(err, 'La IA en la nube no respondió. Intenta de nuevo en unos minutos.') });
            }
            ...
            for (const f of requiredFields) {
                if (!(f in parsed)) {
                    return res.status(502).json({ success: false, error: 'La IA respondió de forma incompleta. Intenta verificar de nuevo.', fieldMissing: f });
                }
            }
            ...
            return res.status(500).json({ success: false, error: mensajeErrorIA(error, 'No se pudo verificar el diagrama con la IA. Intenta de nuevo.') });
        }
    }
