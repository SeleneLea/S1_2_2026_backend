...
class UserSalaController {
    ...
    esDuenoDeSala = async (salaId, requesterId) => {
        ...
        if (parseInt(ownerId, 10) !== parseInt(requesterId, 10)) {
            return { ok: false, status: 403, message: 'No tienes permisos sobre esta sala', sala };
        }
        return { ok: true, sala };
    };
    ...
    register = catchedAsync(async (req, res) => {
        ...
        if (email) {
            ...
            if (!invitedUser) {
                return response(res, 404, { error: true, message: 'No existe un usuario registrado con ese email' });
            }
            invitedId = invitedUser.id;
        }
        if (!invitedId) {
            return response(res, 400, { error: true, message: 'Debe indicar el email o el userId del usuario a invitar' });
        }
        if (parseInt(invitedId, 10) === parseInt(req.user.id, 10)) {
            return response(res, 400, { error: true, message: 'No puedes invitarte a ti mismo' });
        }
        ...
        try {
            ...
            if (error.message && error.message.includes('ya está asociado')) {
                return response(res, 409, { error: true, message: 'El usuario ya tiene acceso a esta sala' });
            }
            throw error;
        }
    });
    ...
    getUserSalaById = catchedAsync(async (req, res) => {
        ...
        if (!usersala) {
            return response(res, 404, { error: true, message: 'Membresía no encontrada' });
        }
